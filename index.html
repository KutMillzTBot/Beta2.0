...
<!-- ================= ROUND ROBIN ROUTING (AI v5) ================= -->
<div class="card mt-4">
  <h2 class="text-xl font-semibold mb-4 text-sky-400">Round Robin Routing</h2>
  <div class="flex items-center justify-between p-3 rounded-lg border border-slate-700 bg-slate-900/30">
    <div>
      <div class="text-sm font-semibold text-slate-200">Round Robin Routing</div>
      <div class="text-xs text-slate-400">
        Rotate symbols sequentially (VOL75 1s → VOL25 1s → VOL10 1s). Default: OFF.
      </div>
    </div>
    <label class="relative inline-flex items-center cursor-pointer">
      <input type="checkbox" id="roundRobinToggle" class="sr-only peer">
      <div class="w-11 h-6 bg-slate-500 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-sky-300 rounded-full peer
        peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px]
        after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-sky-600"></div>
    </label>
  ...
  <div class="mt-3 text-xs text-slate-400">
    When enabled, trades are routed one-by-one across allowed volatility symbols using the same stake and TP.
  </div>
</div>
<!-- =============================================================== -->
...
<script>
// ================= ROUND ROBIN ENGINE (AI v5 – SESSION, DERIV) =================
const DERIV_RR_SYMBOLS = ['VOL75_1S','VOL25_1S','VOL10_1S'];
let rrIndex = 0;

function loadRRFromAI(){
  const rr = window.AI?.executionRouting?.roundRobin;
  if(!rr || !rr.enabled) return;
  rrIndex = rr.currentIndex ?? 0;
}

function saveRRToAI(){
  const rr = window.AI?.executionRouting?.roundRobin;
  if(!rr) return;
  rr.currentIndex = rrIndex;
  saveAI();
}

function advanceRRSessionIfNeeded(){
  const rr = window.AI?.executionRouting?.roundRobin;
  if(!rr || rr.advanceOn !== 'bot_start') return;
  rrIndex = (rrIndex + 1) % DERIV_RR_SYMBOLS.length;
  saveRRToAI();
}

window.rrStats = window.AI?.executionRouting?.roundRobin?.stats || {
  VOL75_1S:{routedTrades:0,wins:0,losses:0},
  VOL25_1S:{routedTrades:0,wins:0,losses:0},
  VOL10_1S:{routedTrades:0,wins:0,losses:0}
};

function isRoundRobinEnabled(){
  const el = document.getElementById('roundRobinToggle');
  return el && el.checked === true;
}

function resolveSymbolForTrade(){
  if(!isRoundRobinEnabled()) return;
  if(typeof appState === 'undefined') return;
  appState.symbol = DERIV_RR_SYMBOLS[rrIndex];
}

function rrTradeOpened(symbol){
  const stats = window.AI?.executionRouting?.roundRobin?.stats;
  if(stats && stats[symbol]){
    stats[symbol].routedTrades++;
    saveAI();
  }
}

function rrTradeClosed(symbol,isWin){
  const stats = window.AI?.executionRouting?.roundRobin?.stats;
  if(!stats || !stats[symbol]) return;
  if(isWin) stats[symbol].wins++;
  else stats[symbol].losses++;
  saveAI();
}

loadRRFromAI();
 (AI v5) =================
const ROUND_ROBIN_SYMBOLS = ['VOL75_1S','VOL25_1S','VOL10_1S'];
let rrIndex = 0;

window.rrStats = {
  VOL75_1S:{trades:0,wins:0,losses:0},
  VOL25_1S:{trades:0,wins:0,losses:0},
  VOL10_1S:{trades:0,wins:0,losses:0}
};
...
function getNextRoundRobinSymbol(defaultSymbol){
  if(!isRoundRobinEnabled()) return defaultSymbol;
  const sym = ROUND_ROBIN_SYMBOLS[rrIndex];
  rrIndex = (rrIndex + 1) % ROUND_ROBIN_SYMBOLS.length;
  return sym;
}

// Call this BEFORE enterTrade()
function resolveSymbolForTrade(){
  if(typeof appState === 'undefined') return;
  appState.symbol = getNextRoundRobinSymbol(appState.symbol);
}

// Stats
function rrTradeOpened(symbol){
  if(rrStats[symbol]) rrStats[symbol].trades++;
}
function rrTradeClosed(symbol,isWin){
  if(!rrStats[symbol]) return;
  if(isWin) rrStats[symbol].wins++;
  else rrStats[symbol].losses++;
}
</script>
<!-- ============================================================ -->


<script src="ai_brain_FINAL.js"></script>
<script>
setInterval(() => {
  if (!window.AI) return;
  const rc = document.getElementById("rrConf");
  const rs = document.getElementById("rrSym");
  const cp = document.getElementById("confPrev");

  if (rc) rc.textContent = AI.getRRConfidence() + "%";
  if (rs) rs.textContent = AI.getCurrentSymbol() + " RR SESSION";
  if (cp) cp.textContent = AI.getConfidencePreview() + "%";
}, 500);
</script>

<div style="position:fixed;bottom:10px;right:10px;z-index:9999">
  <button onclick="AI.exportBrain()">Export AI</button>
  <button onclick="document.getElementById('aiImport').click()">Import AI</button>
  <input id="aiImport" type="file" style="display:none"
    onchange="AI.importBrain(this.files[0])">
</div>
...
<!-- ALL PATCH: chart + RR panels -->
<script>
(function(){
  // Darken canvas charts safely (generic)
...
<div class="ai-extra-panels">
  <h4>RR Confidence</h4>
  <div id="rrConfidence">Reading…</div>
  <h4>Per-Symbol RR Performance</h4>
  <div id="rrPerf">Tracking…</div>
  <h4>Confidence Preview</h4>
  <div id="confPreview">Evaluating…</div>
</div>
...
<style>
.kut-milz-gold {
  color: #e6c36a !important; /* light gold */
  text-shadow: 0 0 6px rgba(230,195,106,0.35);
  font-weight: 700;
...
<!-- REPEATING WATERMARK BACKGROUND PATCH -->
<style>
